...
import { IConfig, UserAgent } from './types';
import { CommandParameters } from './constants';

const getUserAgentArg = (config: IConfig) => {
...
export class CliWrapper {
  ...
  public executeCommand<T extends ClassConstructor<unknown>>(
    ...
    const defaultCliArgs = [CommandParameters.OutputFormatJson, getUserAgentArg(config)];
    ...
    return new Promise((resolve) => {
      ...
      cancellationToken?.onCancellationRequested(() => {
        ...
        resolve(new CliResultPanic(ExitCodes.TERMINATION, 'Command has been cancelled'));
      });
